...
ID3D11ShaderResourceView* TexCache::LoadTexture(ID3D11Device*pDevice, const std::string& fileName, const std::string& texName, 
										bool appendPath, const std::vector<RECTF> *frames)
{
...
DirectX::SimpleMath::Vector2 TexCache::GetDimensions(ID3D11ShaderResourceView* pTex)
{
	assert(pTex);
	...
	DirectX::SimpleMath::Vector2 dim(0, 0);
	if (SUCCEEDED(hr))
	{
