...
bool SubMesh::Initialise(D3D& d3d, const aiScene* scene, const aiMesh* mesh)
{
...
void Mesh::CreateFrom(const std::string& fileName, D3D& d3d)
{
	std::string path, fstring(fileName);
	StripPathAndExtension(fstring, &path);
	if (mName.empty())
		mName = fstring;
	std::string oldPath = d3d.GetCache().GetAssetPath();
